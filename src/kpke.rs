use crate::sample::{sample_ntt, sample_poly_cbd};
use crate::crypto_fns::{g, prf};
use crate::ntt::{ntt, ntt_inv, multiply_ntts};
use crate::byte_fns::{byte_encode, byte_decode, compress, decompress};
use crate::Q;

fn mul_mats<const K:usize>(a:&[[[u16;256];K];K], u:&[[u16;256];K], t:bool) -> [[u16;256];K] {
    core::array::from_fn(|i| {
        let mut acc:[u16;256] = [0;256];
        for j in 0..K {
            let tmp = multiply_ntts(&{if t {a[j][i]} else {a[i][j]}}, &u[j]);
            for i in 0..256 {acc[i] = (acc[i]+tmp[i])%Q}
        }
        acc
    })
}
fn add_mats<const K:usize>(u:&[[u16;256];K], v:&[[u16;256];K]) -> [[u16;256];K] {
    core::array::from_fn(|i| {add_vecs(&u[i], &v[i])})
}
fn dot_prod<const K:usize>(u:&[[u16;256];K], v:&[[u16;256];K]) -> [u16;256] {
    let mut acc:[u16;256] = [0;256];
    for j in 0..K {
        let tmp = multiply_ntts(&u[j], &v[j]);
        for i in 0..256 {acc[i] = (acc[i]+tmp[i])%Q}
    }
    acc
}
fn add_vecs(u:&[u16;256], v:&[u16;256]) -> [u16;256] {
    core::array::from_fn(|i| (u[i] + v[i])%Q)
}

pub(crate) fn keygen<const K:usize, const ETA1_64:usize>(d:&[u8], ek:&mut [u8], dk:&mut [u8]) {
    let eta1 = ETA1_64 as u8/64;
    let k = K as u8;

    let mut d_k = [k;33];
    d_k[..32].clone_from_slice(d);
    let (rho,sigma) = g(&d_k);

    // create new array r <- rho||j||i
    let mut rho_ji: [u8;34] = [0;34];
    rho_ji[..32].clone_from_slice(&rho);
    let a_: [[[u16;256];K];K] = core::array::from_fn(|i|
        core::array::from_fn(|j| {
            // assign i and j to rho_ji
            // return the array from sample_ntt
            // a[i][j] <- sample_ntt(&rho_ji)
            rho_ji[32] = j as u8; rho_ji[33] = i as u8;
            sample_ntt(&rho_ji)
        })
    );

    let mut sigma_n: [u8;33] = [0;33]; // the last element keeps count of N
    sigma_n[..32].clone_from_slice(&sigma);
    let s_:[[u16;256];K] = core::array::from_fn(|_| {
        let x = ntt(&sample_poly_cbd(eta1, &prf::<ETA1_64>(&sigma_n)));
        sigma_n[32] += 1;
        x
    });
    let e_:[[u16;256];K] = core::array::from_fn(|_| {
        let x = ntt(&sample_poly_cbd(eta1, &prf::<ETA1_64>(&sigma_n)));
        sigma_n[32] += 1;
        x
    });

    let t_:[[u16; 256]; K] = add_mats(&mul_mats(&a_, &s_, false), &e_);
    for i in 0..K {
        ek[i*384..i*384+384].clone_from_slice(&byte_encode::<{32*12}>(12, &t_[i]));
    }
    ek[384*K..384*K+32].clone_from_slice(&rho);
    for i in 0..K {
        dk[i*384..i*384+384].clone_from_slice(&byte_encode::<{32*12}>(12, &s_[i]));
    }
}

pub(crate) fn encrypt
<const K:usize, const ETA1_64:usize, const ETA2_64:usize, const DU_32:usize, const DV_32:usize>
(ek:&[u8], m:&[u8;32], r:&[u8;32], c:&mut [u8], du:u8, dv:u8) {
    let eta1 = ETA1_64 as u8/64;
    let eta2 = ETA2_64 as u8/64;
    
    let t_:[[u16;256];K] = core::array::from_fn(|i| byte_decode(12, &ek[384*i..i*384+384]));

    // rho_ji <- rho||j||i
    let mut rho_ji:[u8;34] = [0;34];
    rho_ji[..32].clone_from_slice(&ek[384*K..]);
    let a_: [[[u16;256];K];K] = core::array::from_fn(|i|
        core::array::from_fn(|j| {
            // assign i and j to r
            // return the array from sample_ntt
            // a[i][j] <- sample_ntt(&r)
            rho_ji[32] = j as u8; rho_ji[33] = i as u8;
            sample_ntt(&rho_ji)
        })
    );

    let mut r_n: [u8;33] = [0;33]; // the last element keeps count of N
    r_n[..32].clone_from_slice(r);
    let y_:[[u16;256];K] = core::array::from_fn(|_| {
        let x = ntt(&sample_poly_cbd(eta1, &prf::<ETA1_64>(&r_n)));
        r_n[32] += 1;
        x
    });
    let e1:[[u16;256];K] = core::array::from_fn(|_| {
        let x = sample_poly_cbd(eta2, &prf::<ETA2_64>(&r_n));
        r_n[32] += 1;
        x
    });

    let e2: [u16; 256] = sample_poly_cbd(eta2, &prf::<ETA2_64>(&r_n));

    let tmp = mul_mats(&a_, &y_, true);
    let u: [[u16; 256]; K] = add_mats(&core::array::from_fn(|i| ntt_inv(&tmp[i])), &e1);
    
    let mu:[u16;256] = decompress(1, &byte_decode(1, m));
    let tmp = add_vecs(&ntt_inv(&dot_prod(&t_, &y_)), &e2);
    let v: [u16; 256] = add_vecs(&tmp, &mu);
    
    
    // c1
    for i in 0..K {
        c[i*DU_32..i*DU_32+DU_32].clone_from_slice(&byte_encode::<DU_32>(du,&compress(du, &u[i])));
    }
    // c2
    c[DU_32*K..].clone_from_slice(&byte_encode::<DV_32>(dv,&compress(dv, &v)));
}

pub(crate) fn decrypt<const K:usize>(dk:&[u8], c:&[u8], du:u8, dv:u8) -> [u8;32] {
    let u: [[u16;256];K] = core::array::from_fn(|i: usize| decompress(du, &byte_decode(du, &c[i*32*(du as usize)..(i+1)*32*(du as usize)])));
    let v: [u16;256] = decompress(dv, &byte_decode(dv, &c[32*(du as usize)*K..]));
    let s_: [[u16;256];K] = core::array::from_fn(|i| byte_decode(12, &dk[i*384..i*384+384]));

    let tmp = ntt_inv(&dot_prod(&s_, &core::array::from_fn(|i| ntt(&u[i]))));
    let w:[u16;256] = core::array::from_fn(|i| (v[i] as i16 - tmp[i] as i16).rem_euclid(Q as i16) as u16);
    byte_encode::<32>(1, &compress(1, &w))
}